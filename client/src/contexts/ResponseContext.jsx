...
export const ResponseProvider = ({ children }) => {
  ...
  const updateResponse = async (responseId, updatedContent) => {
    try {
      const response = await api.put(`/responses/${responseId}`, { 
        content: updatedContent.content,
        type: updatedContent.type || 'instagram' 
      });
      
      setResponses(prev => 
        prev.map(r => r._id === responseId ? response.data : r)
      );
  ...
  const value = {
    ...
    saveResponse,
    deleteResponse,
    updateResponse // Add updateResponse to the context value
  };
