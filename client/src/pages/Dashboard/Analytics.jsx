...
//       setError(error.response?.data?.message || "Please connect your Instagram account");
//     } finally {
//       setLoading(false);
//     }
//   };
    ...
    return (
      <div className="flex items-center justify-center min-h-screen bg-gradient-to-br from-indigo-50 via-white to-purple-50">
        <div className="relative p-8 rounded-2xl backdrop-blur-lg bg-white/30">
          ...
          <p className="mt-4 text-indigo-600 font-medium animate-pulse">Loading insights...</p>
        </div>
      </div>
    );
  }
    ...
    <div className={`space-y-8 p-3 bg-gradient-to-br from-indigo-50 via-white to-purple-50 min-h-screen ${isVisible ? 'opacity-100' : 'opacity-0'} transition-all duration-700`}>
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 animate-on-scroll">
        <div>
          ...
          <p className="mt-2 text-gray-600">Track your social media performance and growth</p>
        </div>
      </div>
                ...
                <AreaChart data={insights?.daily_engagement?.slice(-7) || []}>
                  <defs>
                    <linearGradient id={`gradient-${index}`} x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor={metric.positive ? "#4F46E5" : "#DC2626"} stopOpacity={0.2} />
                      <stop offset="100%" stopColor={metric.positive ? "#4F46E5" : "#DC2626"} stopOpacity={0} />
                    </linearGradient>
                  </defs>
                  <Area
                    type="monotone"
            ...
            {topPosts.map((post, index) => (
              <div 
                ...
                className={`transform transition-all duration-300 hover:scale-[1.02] border rounded-xl p-6 bg-gradient-to-br from-white to-indigo-50/30 shadow-sm hover:shadow-md ${
                  hoveredPost === index ? 'border-indigo-300' : 'border-gray-100'
                }`}
