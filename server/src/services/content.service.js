...
try {
  if (!OPENAI_API_KEY) {
    console.error('OpenAI API key is missing. Content generation features will be disabled.');
...
export const generateContentIdeas = async (instagramBusinessId, accessToken, contentType) => {
  try {
    // Get posting analytics to inform content generation
    const analytics = await analyzeBestPostingTimes(instagramBusinessId, accessToken);
    ...
    const completion = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: "system",
          content: "You are an expert Instagram content strategist who creates engaging, trending content ideas."
        },
        {
          role: "user",
          content: prompt
